 ...
 */
class SOrderListActivity : Activity() {
...
//        customer = intent.getParcelableExtra(Constant.Key_Customer)
    ...
    private fun refreshUI() {
        doAsync {
            list.clear()
            list.addAll(YZDDatabase.getDatabase(this@SOrderListActivity).getOrder().query(customer!!.id))
            if (list.isEmpty()) {
                finish()
                return@doAsync
            }
            ...
            for (i in list.indices) {
                ...
                list[i].goodsList.addAll(YZDDatabase.getDatabase(this@SOrderListActivity).getOrderGoods().query(list[i].id))
            ...
            uiThread {
                ...
                if (adapter == null) {
                    adapter = OrderAdapter(this@SOrderListActivity, list)
    ...
    private fun gotoOrderDetail(index: Int) {
        ...
        startActivity<SOrderDetailActivity>()
    }
}
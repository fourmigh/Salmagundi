package org.caojun.salmagundi.lockpattern.widget;
...
//		KLog.e("onMeasure", "(width: " + widthMeasureSpec + "  ,  height: " + heightMeasureSpec + ")");
//		KLog.e("LockPatternView", "(width: " + width + "  ,  height: " + height + ")");
//		if (width != height) {
	...
	private void initViewSize(Context context, AttributeSet attrs){
		for(int i = 0; i < attrs.getAttributeCount(); i ++){
			...
			if("layout_width".equals(name)){
				...
				this.width = LockPatternUtils.changeSize(context, value);
			}
			if("layout_height".equals(attrs.getAttributeName(i))){
				String value = attrs.getAttributeValue(i);
				this.height = LockPatternUtils.changeSize(context, value);
			}
		}
...
//		this.cellBoxHeight = (this.height - offset * 2)/3;
	...
	private void drawTriangle(Cell preCell, Cell nextCell, Canvas canvas, Paint paint) {
		...
		float angleX = LockPatternUtils.getAngleLineIntersectX(
				preCell.getX(), preCell.getY(), nextCell.getX(), nextCell.getY(), distance);
		float angleY = LockPatternUtils.getAngleLineIntersectY(
				preCell.getX(), preCell.getY(), nextCell.getX(), nextCell.getY(), distance);
		float x1, y1, x2, y2;
		//slide right down
	...
	private Cell checkSelectCell(float x, float y) {
		for (int i = 0; i < mCells.length; i++) {
			for (int j = 0; j < mCells[i].length; j++) {
				...
				if (LockPatternUtils.checkInRound(cell.x, cell.y, 80, x, y, this.cellRadius/4)) {
					return cell;
				}
			}
		}
